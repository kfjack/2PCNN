...
SRC_SIG = {'filename':'fatjet_w_match_vz_to_ww.txt.gz', 'skip':60000, 'n':2000}
SRC_BKG = {'filename':'fatjet_q_match_vz_to_qq.txt.gz', 'skip':60000, 'n':2000}
...
model.load_weights(WEIGHTS_2PCNN)

# do performance measurement
ret = []
for batch in range(len(sample)//BATCH_SIZE):

    b_sample = sample[batch*BATCH_SIZE:(batch+1)*BATCH_SIZE]
    max_perm = max([len(x) for x,y,z in b_sample])
    x_deploy = np.array([np.pad(x,((0,max_perm-len(x)),(0,0)),mode='constant',constant_values=0.) for x,y,z in b_sample])
    y_deploy = np.array([y for x,y,z in b_sample])
    z_deploy = np.array([z for x,y,z in b_sample])
    
    pred = model.predict_on_batch([x_deploy,y_deploy])
    loss = K.eval(K.mean(categorical_crossentropy(K.variable(z_deploy),K.variable(pred))))
    acc = (np.argmax(z_deploy,axis=1)==np.argmax(pred,axis=1)).astype('float').mean()
    ret.append([loss,acc])
    for s,p in zip(b_sample,pred): s.append(p)
    
    print('\rbatch: %05d/%05d' % (batch+1,len(sample)//BATCH_SIZE),end='', flush=True)
print('; processing done.', flush=True)
    
deploy_los = np.array(ret)[:,0].mean()
deploy_acc = np.array(ret)[:,1].mean()
print('2PCNN Performance (deployment)', flush=True)
print('Loss: %.5f, Acc: %.5f' % (deploy_los,deploy_acc), flush=True)

if MAKE_SUMMARY_PLOTS:
    import matplotlib.pyplot as plt
    
    # now the stuff stored in sample list is 2pc_input, 2pc_supply, target, prediction
    _2pc_input, _2pc_supply, _target, _prediction = range(4)
    res_sig = [s[_prediction][0]/s[_prediction].sum() for s in sample if np.argmax(s[_target])==0]
    res_bkg = [s[_prediction][0]/s[_prediction].sum() for s in sample if np.argmax(s[_target])==1]
    roc_x = np.array([(res_sig>t).sum()/len(res_sig) for t in np.linspace(0.,1.,200)])
    roc_y = np.array([(res_bkg<t).sum()/len(res_bkg) for t in np.linspace(0.,1.,200)])
    
    fig = plt.figure(figsize=(12,6), dpi=80)
    plt.subplot(121)
    plt.hist(res_sig, bins=120, range=(-0.1,1.1), label='W-jet')
    plt.hist(res_bkg, bins=120, range=(-0.1,1.1), label='q-jet')
    plt.xlabel('2PCNN Score')
    plt.ylabel('Entries')
    plt.grid()
    plt.legend()

    plt.subplot(122)
    plt.plot(roc_x, roc_y, lw=3, label='ROC(2PCNN)',color='C2')
    plt.plot([1,0],[0,1], ls='--', c='gray')
